...
var forbiddenText = [
  { re: /gitea\.grosswig-it\.de/i, label: 'private Gitea host' },
  { re: /harbor\.grosswig-it\.de/i, label: 'private registry host' },
...
function walk(dir) {
  for (var entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (!textExtensions.has(extension) && !entry.name.startsWith('.')) continue;
    var text;
